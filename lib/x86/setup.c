...
void setup_5level_page_table(void)
{
	 ...
	 * some test cases (e.g. x86/access.c) expects it to exist. If this
	 * function is not defined, gcc may generate wrong position-independent
	 * code, which leads incorrect memory access: if compiling
	 * x86/access.efi without this function defined, several data structures
	 * (e.g. apic_ops) get compile time offset memory address, but they
	 * should get runtime %rip based address.
	 *
